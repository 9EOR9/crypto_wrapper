...
{
  unsigned char dst[1000], decrypted[1000];
  int rc= 0;
  ...
  struct t_string *md5= hex2str(test.md5);
   ...
   cw_crypt(test.mode, CW_CRYPT_ENCRYPT | test.flags,
            (unsigned char *)src->str, src->len,
            (unsigned char *)dst, &dlen, key->str, key->len,
            iv->str, iv->len);
  if (exp->len)
  {
    if (dlen != exp->len)
  ...
  if (md5->len)
  {
    ...
    cw_hash(CW_HASH_MD5, md5_str, dst, dlen);
...
#endif
  cw_crypt(test.mode, CW_CRYPT_DECRYPT | test.flags,
           dst, dlen,
  ...
  while (fgets(line, 1024, fp))
  {
    ...
    if ((p= strchr(line, '=')))
    {
      *p= 0;
      ...
      {
        if (strcmp(val, "CW_AES_ECB") == 0)
          test.mode= CW_AES_ECB;
        else if (strcmp(val, "CW_AES_CBC") == 0)
          test.mode= CW_AES_CBC;
        else if (strcmp(val, "CW_AES_GCM") == 0)
          test.mode= CW_AES_GCM;
        else if (strcmp(val, "CW_AES_CTR") == 0)
          test.mode= CW_AES_CTR;
      }
