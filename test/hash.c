...
const char *algorithms[] = { "MD5", "SHA1", "SHA224", "SHA256", "SHA384", "SHA512" };
...
int main()
{
  ...
  {
    if ((ctx= cw_hash_new(my_tests[i].hash_alg)))
    {
      sprintf(test_name, "%s %ld", algorithms[my_tests[i].hash_alg], i);
      for (j=0; j < my_tests[i].repetitions; j++)
      {
        const char *src= my_tests[i].source;
        cw_hash_input(ctx, (unsigned char *)src, strlen(src));
      }
      cw_hash_result(ctx, (unsigned char *)digest);
      ok(compare_results(digest, my_tests[i].digest, cw_hash_digest_size(my_tests[i].hash_alg)),
         test_name);
      cw_hash_free(ctx);
    }
  }
